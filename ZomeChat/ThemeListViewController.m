...
- (void) updateThemeList:(SocketIOPacket *)packet
{
    ...
    APPDELEGATE.mainVC.themeList = [[NSMutableArray alloc] initWithObjects: nil];
    ...
    [APPDELEGATE.mainVC.themeList addObject:popRoom];
    [APPDELEGATE.mainVC.themeList addObject:locRoom];
    ...
    for(int i=0;i<receiveList.count;i++)
    {
        ...
        [APPDELEGATE.mainVC.themeList addObject:[receiveList objectAtIndex:i]];
    }
    [APPDELEGATE.landVC updateRoomCount:[NSString stringWithFormat:@"%lu",(unsigned long)APPDELEGATE.mainVC.themeList.count]];
    [self.tableView reloadData];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    return APPDELEGATE.mainVC.themeList.count;
}
...
//        cellBackground.layer.masksToBounds = YES;
    ...
    NSString *userCountString = [NSString stringWithFormat:@"%@ users",[[APPDELEGATE.mainVC.themeList objectAtIndex:indexPath.row] objectForKey:@"userCount"]];
    userCountLabel.text = userCountString;
    [userCountLabel sizeToFit];
    
    cell.textLabel.text = [[APPDELEGATE.mainVC.themeList objectAtIndex:indexPath.row] objectForKey:@"roomName"];
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row == 0){
        ...
        roomType = @"2";
        roomName = [[APPDELEGATE.mainVC.themeList objectAtIndex:indexPath.row] objectForKey:@"roomName"];
        roomKey = [[APPDELEGATE.mainVC.themeList objectAtIndex:indexPath.row] objectForKey:@"roomKey"];
    }
